...
public class LoginJFrame extends JFrame implements MouseListener {
    ...
    static ArrayList users = new ArrayList<>();  //registered users
	/*
		创建集合初始化一些用户信息
	*/
    static {
    ...
    JButton login = new JButton();  // new login button
    JButton register = new JButton(); //new register button
    JTextField username = new JTextField(); //username textfield
    JPasswordField password = new JPasswordField(); //password textfield
    JTextField code = new JTextField(); //code textfield

    //正确的验证码
    JLabel rightCode = new JLabel(); //show correct code
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if(source == login){
            ...
            } else if (usernameT.length()==0 || pwT.length() == 0) {
                System.out.println("empty name or pw");
                rightCode.setText(CodeUtil.getCode());
            } else if(!codeT.equalsIgnoreCase(rightCode.getText())){
                System.out.println("wrong code");
                rightCode.setText(CodeUtil.getCode());
            } else if (!users.contains(u)) {
                showJDialog("have not registered yet");
                rightCode.setText(CodeUtil.getCode());
            }else{
